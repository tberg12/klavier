...
public class DenseSemiMarkovDP {
	...
	public static List<Pair<Integer,Pair<Integer,Integer>>> viterbiDecode(Model model, int pitchId) {
		...
		for (int t=0; t<model.length(); ++t) {
			if (t == 0) {
				for (int s=0; s<model.numStates(); ++s) {
					for (int w : model.allowedWidths(s)) {
						int nextT = t+w;
						if (nextT <= model.length()) {
							float nodesScore = model.logNodePotential(t, w, s); 
